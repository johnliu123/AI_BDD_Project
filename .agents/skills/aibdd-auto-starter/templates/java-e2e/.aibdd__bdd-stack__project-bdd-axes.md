...
>
> **bdd-stack 即憲法本體**：Cucumber 指令、Spring `@CucumberContextConfiguration` hook、step layout、fixture、feature archive、Red pre-hook 的流程與約束拆在 `.aibdd/bdd-stack/*.md`，並由各 `*_RUNTIME_REF` 與 `${RED_PREHANDLING_HOOK_REF}` 綁定。**本檔不重複**運行手冊或 `arguments.yml` 參數表。
...
3. `${RED_PREHANDLING_HOOK_REF}` → Red Pre-Hook（java-e2e：`schema-analysis`／flyway-migration）；`/aibdd-red-execute` 進 Red 前必讀
