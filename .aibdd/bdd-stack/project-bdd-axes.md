...
>
> **bdd-stack 即憲法本體**：Behave 指令、environment hook、step layout、fixture、feature archive、Red pre-hook 的流程與約束拆在 `.aibdd/bdd-stack/*.md`，並由各 `*_RUNTIME_REF` 與 `.aibdd/bdd-stack/prehandling-before-red-phase.md` 綁定。**本檔不重複**運行手冊或 `arguments.yml` 參數表。
...
3. `.aibdd/bdd-stack/prehandling-before-red-phase.md` → Red Pre-Hook（python-e2e：`schema-analysis`／data-migration）；`/aibdd-red-execute` 進 Red 前必讀
...
**說明**：規格與測試目錄皆在 repo 內固定路徑；預設**不**對 `specs/packages/<<NN-functional-module>>/features` → `tests/features/` 做自動搬遷。實際 RED／ symlink／鏡像流程見 `bdd-stack/feature-archive.md`。若未來改機制，須先修訂本節與 `arguments.yml`。
