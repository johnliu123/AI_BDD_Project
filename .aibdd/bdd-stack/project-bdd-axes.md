...
**說明**：規格與測試目錄皆在 repo 內固定路徑；預設**不**對 `specs/packages/<<NN-functional-module>>/features` → `src/test/resources/features/` 做自動搬遷。實際 RED／symlink／鏡像流程見 `bdd-stack/feature-archive.md`。若未來改機制，須先修訂本節與 `arguments.yml`。
